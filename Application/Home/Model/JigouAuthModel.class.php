...
class JigouAuthModel extends Model{
    //权限模型涉及表有多个,关闭表检测
    ...
    public function query_rule($condition='',$multi=TRUE){
        $db = M('jigou_rule');
        if($multi){
            if(!empty($condition)){return $db->where($condition)->field('t2.title as modname,hk_jigou_rule.*')->join('left join hk_jigou_module t2 ON t2.id=hk_jigou_rule.mid')->select();}
            return $db->field('t2.title as modname,hk_jigou_rule.*')->join('left join hk_jigou_module t2 ON t2.id=hk_jigou_rule.mid')->select();
        }else{
            if(!empty($condition)){return $db->where($condition)->field('t2.title as modname,hk_jigou_rule.*')->join('left join hk_jigou_module t2 ON t2.id=hk_jigou_rule.mid')->find();}
            return $db->field('t2.title as modname,hk_jigou_rule.*')->join('left join hk_jigou_module t2 ON t2.id=hk_jigou_rule.mid')->find();
        }
    }
